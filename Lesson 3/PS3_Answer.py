...
is a reasonable low bound for this payment value? $0 is the obvious anwer, but you can do better than that. If there was no interest,
...
One-twelfth of the original balance is a good low bound.
...
Monthly payment low bound = Balance / 12
...
low = init_balance/12
upper = (init_balance * (1 + monthInt)**12)/12.0
epsilon = 0.03

while abs(balance) > epsilon:
    monthInt = (upper + low)/2
    balance = init_balance
    for i in range(12):
        balance = balance - monthInt + ((balance - monthInt) * monthInt)
    if balance > epsilon:
        low = monthInt
        ...
        break
print('Lowest Payment:', round(monthInt, 2))  